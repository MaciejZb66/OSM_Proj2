...
void TFTDisplay_ILI9341_FillCircle(uint16_t x_Position, uint16_t y_Position, int radius, uint16_t color){
    int f = 1 - radius;
    int ddF_x = 1;
    int ddF_y = -2 * radius;
    int x = 0;
    ...
    // Środek i pierwsze pionowe linie
    TFTDisplay_ILI9341_DrawLine(x_Position - radius, y_Position, x_Position + radius, y_Position, color);
    ...
    while (x < y)
    {
        ...
        // Rysujemy poziome linie między punktami po obu stronach "okręgu"
        TFTDisplay_ILI9341_DrawLine(x_Position - x, y_Position + y, x_Position + x, y_Position + y, color);
        TFTDisplay_ILI9341_DrawLine(x_Position - x, y_Position - y, x_Position + x, y_Position - y, color);
        TFTDisplay_ILI9341_DrawLine(x_Position - y, y_Position + x, x_Position + y, y_Position + x, color);
        TFTDisplay_ILI9341_DrawLine(x_Position - y, y_Position - x, x_Position + y, y_Position - x, color);
    }
